...
//        window.location.href = "../ingresar/ingresar.html";
        ...
        jQuery.ajax({
            ...
            'type': 'POST',
            'url': "../api/producto/getFromID" + "/" + "?q=" + (new Date()).getTime(),
            'data': Productos,
            'dataType': 'json',
            'success': mostrarContactos
        });
...
//     var fields = nroCompra.split('~');
//    var productoId = fields[0];
//    var precio = fields[1];
//    var fecha = new Date();
...
//    var pedido = new Object();
    ...
    var cliente = sessionStorage.getItem("usuarioId");
    pedido.fecha = fecha;
    pedido.usuarioId = cliente;
    var arrayDetalle = [];
    var totalDolares = 0;

    $(".Lista_Busqueda li").each(function (index) {
...
//    private int subtotal;
//    private int precio;
//                                 {productoId: productoId, cantidad: 1, subtotal: precio, precio: precio}
            var detallePequeño = {productoId: productoId.substring(1), cantidad: 1, subtotal: precio, precio: precio};
            arrayDetalle[index] = detallePequeño;
            totalDolares += parseInt(precio);

            var favoritos = JSON.parse(localStorage.getItem("carrito"));
            delete favoritos[productoId.substring(1)];
            localStorage.setItem("carrito", JSON.stringify(favoritos));


        }
    });
    pedido.detalle = arrayDetalle;
    pedido.total = totalDolares;
//    debugger;
    ...
    jQuery.ajax({
        ...
        'type': 'POST',
        'url': "../api/pedido/RealizarCompra" + "/" + "?q=" + (new Date()).getTime(),
...
function RealizarCompraParcial(nroCompra) {
//    debugger;
    // compra parcial es la de 
//    alert("Hola");
    var fields = nroCompra.split('~');
    var productoId = fields[0];
    var precio = fields[1];
...
function respuesta2(valor) { // 1 bien 2 no bien
    var nume = valor.nro
    if (nume > 1) { // perfect 
        alert("Horario Ocupado");
        //   $("#respuesta").html("Todo Ok");    
...
//    debugger;
    ...
    if (lista.length < 1) {
        $(".descripcionAuto").append("Su Carrito Esta vacio");
    } else {
        $(".descripcionAuto").append("Vehiculos en su Carrito");
    }
    ...
    for (var i in lista) {
        ...
        var strComprar = "onclick= \"RealizarCompraParcial('" + obj.productoId + "~" + obj.precio + "')\" >Realizar Compra Parcial</button>";
        var strTodo = " onclick= \"ComprarTodo()\" >Comprar Todo </button>";
        var favoritos = JSON.parse(localStorage.getItem("carrito"));
        if (favoritos[obj.productoId])
            strAgregar = strQuitar
        var strVar = "";
        strVar += "					<li id=\"" + "E" + obj.productoId + "~" + obj.precio + "\" >";
...
//        strVar += "							<button id=\"testdrive\" onclick=\"testDrive(" + obj.productoId + ")\" type=\"button\">Programar Test Drive</button>";
        ...
        strVar += "							<button id=\"realizarCompra" + obj.productoId + "\" type=\"button\" " + strComprar;
        strVar += "						<\/form>							";
        strVar += "					<\/li>";
//             $(".Lista_Busqueda")               
        $(".Lista_Busqueda").append(strVar);
    }
    $(".Lista_Busqueda").append("<li> <button id=\"ComprarTodo\"" + strTodo + " <\/li>");
